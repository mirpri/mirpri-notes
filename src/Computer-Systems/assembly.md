...
| 变址寻址    | movl ==(%ebx,%esi,4)==, %eax     | 地址偏移量为：常数+寄存器\*F           |
...
:::info AT&T
操作数的字长由操作符的最后一个字母决定，后缀‘b’、‘w’、‘l’分别表示操作数为字节、字和双字
:::
...
`call`: EIP存入ESP的地址，ESP-n （栈向低地址增长）
`ret`: ESP指向的地址送给EIP，ESP+n
...
0x...e018       [ func 局部变量/工作区  ] <--- RSP = 0x...e018 (func 的栈顶)

------------------------------------------------------------------------
【ALU 是怎么拿到参数的？】:
子程序内部执行：`movl 16(%rbp), %eax`（捞出 10）和 `addl 24(%rbp), %eax`（加上 20）。
参数从【堆栈内存】出发，通过【数据总线】，流入 CPU 内部。
```
...
- `INT`：软中断，EFLAGS入栈，断点（CS和RIP）入栈，TF设为0，若中断门将IF设为0
